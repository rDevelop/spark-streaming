  ...
  def main(args: Array[String]) {
    ...
    lengths.foreachRDD((rdd, time) => {
      ...
      if (count > 0) {
        ...
        // REDUCE
        // combine everything to one final value. 
        // two sub totals and add together
        //totalChars.getAndAdd(rdd.reduce((x,y) => x + y))
        
        var chars = rdd.reduce((x,y) => x + y)
        var tweetLength = chars / count
        
        if(tweetLength > longestTweet.get()) {
          println("New longest tweet " + tweetLength)
          longestTweet.set(tweetLength)
        }
